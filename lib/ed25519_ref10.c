...
static int
_crypto_core_ed25519_from_uniform(unsigned char *p, const unsigned char *r)
{
...
CAMLprim value
spoke_ed25519_from_uniform (value dst, value dst_off, value src, value src_off) {
  _crypto_core_ed25519_from_uniform(Bytes_off (dst, 0), String_off (src, src_off));
}
                           ...
                           const unsigned char *p, const int clamp)
{
    ...
    if (_crypto_scalarmult_ed25519_is_inf(q) != 0 || is_zero(n, 32)) {
        return -1;
    }
    return 0;
}
...
CAMLprim value
spoke_ed25519_scalarmult_base_noclamp (value dst, value dst_off, value hash, value hash_off) {
  CAMLparam4(dst, dst_off, hash, hash_off);
  _crypto_scalarmult_ed25519_base(Bytes_off (dst, dst_off), String_off (hash, hash_off), 0);
  CAMLreturn (Val_unit);
}

CAMLprim value
spoke_ed25519_scalarmult_base (value dst, value dst_off, value hash, value hash_off) {
  CAMLparam4(dst, dst_off, hash, hash_off);
  _crypto_scalarmult_ed25519_base(Bytes_off (dst, dst_off), String_off (hash, hash_off), 1);
  CAMLreturn (Val_unit);
}

CAMLprim value
spoke_ed25519_scalarmult_noclamp (value dst, value hash, value hash_off, value point) {
  CAMLparam4(dst, hash, hash_off, point);
  int res = _crypto_scalarmult_ed25519(Bytes_val (dst), String_off (hash, hash_off), String_val (point), 0);
  CAMLreturn (Val_bool (res == 0));
}

CAMLprim value
spoke_ed25519_scalarmult (value dst, value hash, value hash_off, value point) {
  CAMLparam4(dst, hash, hash_off, point);
  int res = _crypto_scalarmult_ed25519(Bytes_val (dst), String_off (hash, hash_off), String_val (point), 1);
  CAMLreturn (Val_bool (res == 0));
}

static int
_crypto_core_ed25519_add(unsigned char *r,
...
static int
_crypto_core_ed25519_sub(unsigned char *r,
                        const unsigned char *p, const unsigned char *q)
{
    ge25519_p3     p_p3, q_p3, r_p3;
    ge25519_p1p1   r_p1p1;
...
CAMLprim value
spoke_ed25519_add(value dst, value dst_off, value f, value g) {
  CAMLparam4(dst, dst_off, f, g);
  _crypto_core_ed25519_add (Bytes_off (dst, dst_off),
		            String_val (f),
			    String_val (g));
  CAMLreturn (Val_unit);
}
...
CAMLprim value
spoke_ed25519_sub(value dst, value dst_off, value f, value g) {
  CAMLparam4(dst, dst_off, f, g);
  _crypto_core_ed25519_sub (Bytes_off (dst, dst_off),
		            String_val (f),
			    String_val (g));
  CAMLreturn (Val_unit);
}
